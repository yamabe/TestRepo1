  ...
  return {
    ...
    init: function init(id, op, callback) {
      ...
      if (typeof this.ids !== 'object') { //1th
        ...
        if (isModule){
          for (var i in this.m) {
            if(this.m[i])this.extend(this.m[i]);
          }
        }
      }
      ...
      */
      ...
      if(typeof this.op.config.axisXLen === 'number'){
        ...
        if(typeof this.gcf.axisXLen === 'number'){
          ...
          //Y目盛を小数点無しで maxY < axisXLen なら水平目盛り線の数を maxYまで減らす
          if(this.yScaleDecimal === 'no'){
            if(this.maxY < this.axisXLen) this.axisXLen = parseInt(this.maxY);
          }
        }
      }
