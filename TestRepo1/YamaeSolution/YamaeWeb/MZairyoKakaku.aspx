<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MZairyoKakaku.aspx.cs" Inherits="MZairyo" %>
...
<y:YDropDownList   id="材料属性ID"  Label="材料属性ID"  IsGrid="false"  ReadOnly="TRUE" SelectedValue='<%# Bind("材料属性ID") %>'   IsRequired="false" IsDate="false"  DataFormatString="" IsInteger="true" runat="server" CssClass="Input"  DataSourceID="材料属性DataSource" DataTextField="材料名称" DataValueField="材料属性ID"   AppendDataBoundItems="true"   Width="70"/>
...
<y:YTextBox   id="最終更新日時"  Label="最終更新日時"  IsGrid="false"  ReadOnly="TRUE" Text='<%# Bind("最終更新日時", "{0:yyyy/MM/dd hh:mm:ss}") %>'    IsRequired="FALSE" IsDate="true"  DataFormatString="{0:yyyy/MM/dd hh:mm:ss}" IsInteger="false" runat="server" CssClass="Auto"   Width="100"/>
            ...
            <y:BaseSqlDataSource ID="mainDataSource" runat="server"
                ...
                材料属性ID = @材料属性ID,材料名称 = @材料名称,材料メーカー = @材料メーカー,材質大分類 = @材質大分類,材質 = @材質,定尺寸法縦 = @定尺寸法縦,定尺寸法横 = @定尺寸法横,厚み = @厚み,密度 = @密度,定尺仕入金額 = @定尺仕入金額,m2あたり材料費 = @m2あたり材料費,定尺売り金額 = @定尺売り金額,利益率 = @利益率,有効フラグ = @有効フラグ,削除フラグ = @削除フラグ,作成ユーザー = @作成ユーザー,最終更新ユーザー = @最終更新ユーザー,作成日時 = @作成日時,最終更新日時 = @最終更新日時 
                where 材料ID = @original_材料ID and 最終更新日時 = @original_最終更新日時"
                InsertCommand=" insert into m材料価格 
                (材料属性ID,材料名称,材料メーカー,材質大分類,材質,定尺寸法縦,定尺寸法横,厚み,密度,定尺仕入金額,m2あたり材料費,定尺売り金額,利益率,有効フラグ,削除フラグ,作成ユーザー,最終更新ユーザー,作成日時,最終更新日時)
                 values 
                (@材料属性,@材料名称,@材料メーカー,@材質大分類,@材質,@定尺寸法縦,@定尺寸法横,@厚み,@密度,@定尺仕入金額,@m2あたり材料費,@定尺売り金額,@利益率,@有効フラグ,@削除フラグ,@作成ユーザー,@最終更新ユーザー,@作成日時,@最終更新日時);
